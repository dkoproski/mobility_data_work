...
run_nb_model = function(df, category_name, category_vector, reference_name, target_categories) {
  ...
  df_model = df_long |> 
    filter(top_category %in% c(target_categories, all_cat)) |>  # Filter to only relevant POIs
    mutate(category_indicator = if_else(top_category %in% target_categories, "Restaurant/Bar", "Non-Restaurant"))
