...
## Healthcare categories vs. categories of interest

```{r}
df_long_model_filtered_2 = df_long |> 
  filter(top_category %in% c(target_categories, medical_services)) |> 
  mutate(essential = if_else(top_category %in% target_categories, "No", "Yes"))

poisson_model_interact_2 = glm(visitor_count ~ age_group * essential, family = poisson(link = "log"), data = df_long_model_filtered_2)
summary(poisson_model_interact_2)
```

## NB models, overdispersion was present

```{r}
run_nb_model = function(df, category_name, category_vector, reference_name) {
  df_model = df |> 
    mutate(category_indicator = if_else(top_category %in% category_vector, reference_name, category_name))
...
# Run models for each category comparison
nb_model_1 = run_nb_model(df_long, "Non-Restaurant", target_categories, "Restaurant/Bar")

nb_model_2 = run_nb_model(df_long, "Healthcare", medical_services, "Restaurant/Bar")

nb_model_3 = run_nb_model(df_long, "Essential Services", essential_services, "Restaurant/Bar")

nb_model_4 = run_nb_model(df_long, "Retail Shopping", retail_shopping, "Restaurant/Bar")

nb_model_5 = run_nb_model(df_long, "Entertainment/Recreation", entertainment_recreation, "Restaurant/Bar")

nb_model_6 = run_nb_model(df_long, "Personal Services", personal_services, "Restaurant/Bar")

nb_model_7 = run_nb_model(df_long, "Hospitality/Lodging", hospitality_lodging, "Restaurant/Bar")

nb_model_8 = run_nb_model(df_long, "Office/Professional", office_professional, "Restaurant/Bar")

```

## NB result extraction

```{r}
extract_nb_results = function(model, category_name) {
  results = broom.mixed::tidy(model) |> 
    filter(grepl("Intercept|age_group|category_indicator", term)) |> 
